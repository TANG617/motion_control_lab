...
constexpr const char *kMujocoModelEnvironmentVariable = "MCL_MUJOCO_MODEL";
...
void validate(const HierarchicalOptions &options) {
  ...
  if (options.simulation.mujoco_model_path.empty()) {
    throw std::runtime_error(
        "--mujoco-model is required unless MCL_MUJOCO_MODEL is set");
  }
...
void printHierarchicalUsage(const char *program) {
  ...
  std::cout
      ...
      << "  --mujoco-model <path> MuJoCo MJCF model (or $MCL_MUJOCO_MODEL)\n"
...
HierarchicalOptions parseHierarchicalOptions(int argc, char **argv) {
  ...
  options.simulation.mujoco_model_path = MCL_R1_MUJOCO_MODEL_PATH;
  if (const char *configured_urdf =
          std::getenv(kMotionControlUrdfEnvironmentVariable)) {
    options.urdf_path = configured_urdf;
  }
  if (const char *configured_model =
          std::getenv(kMujocoModelEnvironmentVariable)) {
    options.simulation.mujoco_model_path = configured_model;
  }
