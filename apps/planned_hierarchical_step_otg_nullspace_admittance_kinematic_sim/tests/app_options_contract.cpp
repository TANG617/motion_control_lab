...
int main() {
  char program[] = "mcl_planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim";
  char *asset_defaults_argv[]{program};
  const auto asset_defaults =
      app::parseHierarchicalOptions(1, asset_defaults_argv);
  if (asset_defaults.robot.collision_mesh_search_paths.size() != 1U ||
      std::filesystem::path{
          asset_defaults.robot.collision_mesh_search_paths.front()} !=
          std::filesystem::weakly_canonical(asset_defaults.urdf_path)
              .parent_path()) {
    return EXIT_FAILURE;
  }
  ...
  char mujoco_option[] = "--mujoco-model";
  char mujoco_model[] = "/tmp/r1.xml";
  char no_viewer[] = "--no-mujoco-viewer";
  char angular_admittance[] = "--angular-admittance";
  char filter_option[] = "--wrench-filter-alpha";
  char filter_alpha[] = "0.5";
  char *custom_argv[]{program,       urdf_option, urdf,      red_option, red,
                      ...
                      joint_weights_option, joint_weights, mujoco_option,
                      mujoco_model, no_viewer, angular_admittance,
                      filter_option, filter_alpha};
  const auto custom = app::parseHierarchicalOptions(21, custom_argv);
  if (custom.red_rate_hz != 200.0 || custom.yellow_rate_hz != 40.0 ||
      ...
      custom.simulation.mujoco_model_path != "/tmp/r1.xml" ||
      custom.simulation.viewer_enabled || !custom.admittance.angular_enabled ||
      custom.admittance.wrench_filter_alpha != 0.5) {
    return EXIT_FAILURE;
  }
  ...
  return help.str().find("default: 1000") != std::string::npos &&
                 ...
                 help.str().find("--joint-synchronization") !=
                     std::string::npos
                 && help.str().find("--mujoco-model") != std::string::npos &&
                 help.str().find("--wrench-filter-alpha") != std::string::npos
             ? EXIT_SUCCESS
             : EXIT_FAILURE;
}
