...
void ReplayPipelineGate::pause() noexcept {
  paused_.store(true, std::memory_order_release);
  red_tick_budget_.store(0U, std::memory_order_release);
  yellow_tick_budget_.store(0U, std::memory_order_release);
}

void ReplayPipelineGate::resume() noexcept {
  red_tick_budget_.store(0U, std::memory_order_release);
  yellow_tick_budget_.store(0U, std::memory_order_release);
...
bool ReplayPipelineGate::acquireTick(
    std::atomic<std::uint64_t> &budget) noexcept {
  auto remaining = budget.load(std::memory_order_acquire);
  while (remaining > 0U) {
    if (budget.compare_exchange_weak(remaining, remaining - 1U,
...
bool ReplayPipelineGate::acquireRedTick() noexcept {
  return !paused_.load(std::memory_order_acquire) ||
         acquireTick(red_tick_budget_);
}

bool ReplayPipelineGate::acquireYellowTick() noexcept {
  return !paused_.load(std::memory_order_acquire) ||
         acquireTick(yellow_tick_budget_);
}

bool ReplayPipelineGate::paused() const noexcept {
  return paused_.load(std::memory_order_acquire);
}
...
constexpr const char *kProgramId =
    "mcl_planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim";
...
const char *priorityName(mcc::PriorityLevel value) {
  switch (value) {
  case mcc::PriorityLevel::Primary:
    return "Primary";
  case mcc::PriorityLevel::Secondary:
    return "Secondary";
  case mcc::PriorityLevel::Tertiary:
    return "Tertiary";
  }
  return "unknown";
}
...
void updateQpPassTimePercentiles(mcl::SolverDebug &output,
...
void updateSolverDebug(mcl::SolverDebug &output,
  ...
  if (diagnostics.hierarchical) {
    ...
    for (const auto &pass : diagnostics.hierarchy.passes) {
      output.warm_start_used = output.warm_start_used || pass.warm_start_used;
...
struct DragSnapshot {
  std::array<mcs::DragReference, 2> references;
};
...
struct RedOutputSnapshot {
  ...
  mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::JointTarget raw_joint_target;
  mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::JointTarget
      projected_joint_target;
  mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::ProjectionDiagnostics
      projection;
  ...
  mcc::Pose raw_right_link4_pose{mcc::Pose::Identity()};
  ...
  DualAdmittanceOutput admittance;
...
struct RedAttemptSnapshot {
  ...
  mcl::SolverDebug solver_debug;
  mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::RetargetClampDiagnostics
      retarget_clamp;
  std::uint64_t retarget_clamp_target_revision{0U};
  std::string detail;
};

mcs::Pose3d simulationPose(const mcc::Pose &pose) {
  const Eigen::Quaterniond orientation(pose.linear());
  mcs::Pose3d result;
  result.position_m = {
      {pose.translation().x(), pose.translation().y(), pose.translation().z()}};
  result.orientation_wxyz = {
...
mcs::JointState simulationJointState(
  ...
  result.positions = toStdVector(state.positions);
  result.velocities = toStdVector(state.velocities);
  result.sample_time_seconds =
      static_cast<double>(state.monotonic_time_nanoseconds) * 1.0e-9;
  return result;
}

DragSnapshot initialDragSnapshot(const RedOutputSnapshot &output,
                                 const mcl::R1RobotConfig &robot) {
  DragSnapshot result;
  result.references[0].id = "left";
  result.references[0].target_pose =
      simulationPose(output.left_pose * robot.left_tcp_offset);
  result.references[1].id = "right";
  result.references[1].target_pose =
      simulationPose(output.right_pose * robot.right_tcp_offset);
  return result;
}
...
DualAdmittanceOutput initialAdmittanceOutput(
  ...
  initialize_arm(nominal_sample.frames.at(0), left_actual_link_pose,
                 robot.left_tcp_offset, output.left);
  initialize_arm(nominal_sample.frames.at(1), right_actual_link_pose,
                 robot.right_tcp_offset, output.right);
  output.command_sample = nominal_sample;
  return output;
}

std::vector<mcs::ArrowOverlay>
viewerWrenchArrows(const DualAdmittanceOutput &admittance) {
  constexpr double kForceArrowScaleMPerN = 0.005;
  std::vector<mcs::ArrowOverlay> result;
  result.reserve(2U);
  const auto append = [&](const ArmAdmittanceOutput &arm,
                          std::array<float, 4> color) {
    mcs::ArrowOverlay arrow;
    for (std::size_t axis = 0; axis < 3; ++axis) {
      arrow.start_m[axis] =
          arm.actual_tcp_pose.translation()[static_cast<Eigen::Index>(axis)];
      arrow.end_m[axis] =
          arrow.start_m[axis] +
          kForceArrowScaleMPerN *
              arm.filtered_wrench.force[static_cast<Eigen::Index>(axis)];
    }
    arrow.color_rgba = color;
    arrow.width = 0.008;
    result.push_back(arrow);
  };
  append(admittance.left, {{1.0F, 0.55F, 0.05F, 1.0F}});
  append(admittance.right, {{0.1F, 0.55F, 1.0F, 1.0F}});
  return result;
}

bool anySaturation(const mcc::CartesianAdmittanceDiagnostics &diagnostics) {
  return std::any_of(diagnostics.velocity_saturated.begin(),
                     diagnostics.velocity_saturated.end(),
                     [](bool value) { return value; }) ||
         std::any_of(diagnostics.offset_saturated.begin(),
                     diagnostics.offset_saturated.end(),
                     [](bool value) { return value; });
}

void appendAdmittanceVisualization(const DualAdmittanceOutput &admittance,
                                   std::uint64_t timestamp_ns,
                                   motion_control::viz::RenderBatch &batch) {
  namespace mcv = motion_control::viz;
  constexpr double kForceArrowScaleMPerN = 0.005;
  const auto append = [&](const ArmAdmittanceOutput &arm,
                          const std::string &side, mcv::ColorRgba color) {
    const auto append_pose = [&](const mcc::Pose &source,
                                 const std::string &channel) {
      const Eigen::Quaterniond orientation(source.linear());
      mcv::PoseSample pose;
    ...
    append_pose(arm.nominal_tcp.pose,
    ...
    wrench.channel = "/mcl/admittance/" + side + "/wrench";
    wrench.frame_id = "base_link";
    wrench.points_m = {
        {{{arm.actual_tcp_pose.translation().x(),
           arm.actual_tcp_pose.translation().y(),
           arm.actual_tcp_pose.translation().z()}},
         {{arm.actual_tcp_pose.translation().x() +
               kForceArrowScaleMPerN * arm.filtered_wrench.force.x(),
           arm.actual_tcp_pose.translation().y() +
               kForceArrowScaleMPerN * arm.filtered_wrench.force.y(),
           arm.actual_tcp_pose.translation().z() +
               kForceArrowScaleMPerN * arm.filtered_wrench.force.z()}}}};
    wrench.color = color;
    wrench.thickness = 4.0;
    batch.line_strips.push_back(std::move(wrench));

    mcv::LineStrip3d offset;
    offset.entity_id = side + "_admittance_offset";
    offset.channel = "/mcl/admittance/" + side + "/offset";
    offset.frame_id = "base_link";
    offset.points_m = {{{{arm.nominal_tcp.pose.translation().x(),
                          arm.nominal_tcp.pose.translation().y(),
                          arm.nominal_tcp.pose.translation().z()}},
                        {{arm.command_tcp.pose.translation().x(),
                          arm.command_tcp.pose.translation().y(),
                          arm.command_tcp.pose.translation().z()}}}};
    offset.color = color;
    offset.thickness = 3.0;
    batch.line_strips.push_back(std::move(offset));
  ...
  append(admittance.right, "right", {0.1, 0.55, 1.0, 1.0});
}

const char *retargetClampComponentName(
    mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::RetargetClampComponent
        component) {
  using Component =
      mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::RetargetClampComponent;
  switch (component) {
...
const char *compactJointName(std::size_t index) {
  constexpr std::array<const char *, 20> kNames{
      "HY", "HP", "TY", "TP", "KP", "AP", "L1", "L2", "L3", "L4",
      "L5", "L6", "L7", "R1", "R2", "R3", "R4", "R5", "R6", "R7"};
  return kNames.at(index);
}

const char *
projectionFlag(mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::ProjectionComponent
                   component) {
  using Component =
      mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::ProjectionComponent;
  switch (component) {
...
proto::SolverTelemetry makeSolverTelemetry(const proto::SampleContext &context,
                                           const char *solver_kind,
                                           const SolverDiagnostics &diagnostics,
                                           const mcl::SolverDebug &debug,
                                           const SolverOptions &options) {
  ...
  if (diagnostics.hierarchical) {
    ...
    for (const auto &source : diagnostics.hierarchy.passes) {
      ...
      for (const auto &violation : source.constraint_violations) {
        ...
        output->set_label(violation.source_name + "." +
...
proto::CouplingTelemetry
makeCouplingTelemetry(const proto::SampleContext &context,
                      const SolverDiagnostics &diagnostics,
  ...
  *message.mutable_context() = context;
  ...
  if (captured_time >= 0 &&
      stamp.run_time_ns >= static_cast<std::uint64_t>(captured_time)) {
    message.set_source_age_ms(
        static_cast<double>(stamp.run_time_ns -
                            static_cast<std::uint64_t>(captured_time)) /
        1.0e6);
  }
...
proto::WorkerTelemetry
makeWorkerTelemetry(const proto::SampleContext &context, const char *role,
                    double rate_hz, const mcl::PeriodicIterationTiming &timing,
...
void setCartesianError(proto::CartesianError &output, const mcc::Pose &target,
                       const mcc::Pose &actual) {
  const Eigen::Vector3d position = target.translation() - actual.translation();
  const Eigen::AngleAxisd rotation(target.linear() *
                                   actual.linear().transpose());
...
proto::CartesianTracking makeCartesianTracking(
  ...
  *message.mutable_context() = context;
  const auto append = [&](const char *side, const mcc::Pose &arm_goal,
                          const mcc::Pose &arm_reference, const mcc::Pose &raw,
                          const mcc::Pose &executed, std::size_t frame_index) {
    auto *arm = message.add_arms();
    arm->set_side(side);
    setCartesianError(*arm->mutable_goal_to_reference(), arm_goal,
                      arm_reference);
    setCartesianError(*arm->mutable_reference_to_ik(), arm_reference, raw);
    setCartesianError(*arm->mutable_reference_to_execution(), arm_reference,
                      executed);
...
proto::CartesianCompliance makeCartesianCompliance(
  ...
  *message.mutable_context() = context;
  const auto append_wrench = [](proto::CartesianWrench &target,
                                const mcc::Wrench &source) {
  ...
  const auto append = [&](const char *side,
    ...
    arm->set_control_point_frame_name(source.nominal_tcp.frame_name);
    setCartesianError(*arm->mutable_nominal_to_command(),
                      source.nominal_tcp.pose, source.command_tcp.pose);
    setVector3(*arm->mutable_nominal_linear_velocity_mps(),
               source.nominal_tcp.twist.head<3>());
    setVector3(*arm->mutable_nominal_angular_velocity_radps(),
               source.nominal_tcp.twist.tail<3>());
    setVector3(*arm->mutable_nominal_linear_acceleration_mps2(),
               source.nominal_tcp.acceleration.head<3>());
    setVector3(*arm->mutable_nominal_angular_acceleration_radps2(),
               source.nominal_tcp.acceleration.tail<3>());
    setVector3(*arm->mutable_command_linear_velocity_mps(),
               source.command_tcp.twist.head<3>());
    setVector3(*arm->mutable_command_angular_velocity_radps(),
               source.command_tcp.twist.tail<3>());
    setVector3(*arm->mutable_command_linear_acceleration_mps2(),
               source.command_tcp.acceleration.head<3>());
    setVector3(*arm->mutable_command_angular_acceleration_radps2(),
               source.command_tcp.acceleration.tail<3>());
    append_wrench(*arm->mutable_raw_wrench(), source.raw_wrench);
    append_wrench(*arm->mutable_filtered_wrench(), source.filtered_wrench);
    append_wrench(*arm->mutable_applied_wrench(),
                  source.diagnostics.applied_wrench);
    setVector3(*arm->mutable_compliance_translation_m(),
               source.diagnostics.compliance_offset.head<3>());
    setVector3(*arm->mutable_compliance_rotation_vector_rad(),
               source.diagnostics.compliance_offset.tail<3>());
    setVector3(*arm->mutable_compliance_linear_velocity_mps(),
               source.diagnostics.compliance_twist.head<3>());
    setVector3(*arm->mutable_compliance_angular_velocity_radps(),
               source.diagnostics.compliance_twist.tail<3>());
    setVector3(*arm->mutable_compliance_linear_acceleration_mps2(),
               source.diagnostics.compliance_acceleration.head<3>());
    setVector3(*arm->mutable_compliance_angular_acceleration_radps2(),
               source.diagnostics.compliance_acceleration.tail<3>());
    for (bool saturated : source.diagnostics.velocity_saturated) {
      arm->add_velocity_saturated(saturated);
    }
    for (bool saturated : source.diagnostics.offset_saturated) {
      arm->add_offset_saturated(saturated);
    }
    arm->set_drag_active(source.drag_active);
    setVector3(*arm->mutable_actual_linear_velocity_mps(),
               source.actual_tcp_twist.head<3>());
    setVector3(*arm->mutable_actual_angular_velocity_radps(),
               source.actual_tcp_twist.tail<3>());
  };
  append("left", compliance.left);
  append("right", compliance.right);
  return message;
}

proto::JointTracking
makeJointTracking(const proto::SampleContext &context,
                  const std::vector<std::string> &joint_names,
                  const RedOutputSnapshot &output,
                  const JointTargetLimits &limits) {
  proto::JointTracking message;
  *message.mutable_context() = context;
  std::vector<std::vector<proto::ProjectionKind>> projection_kinds(
      joint_names.size());
  ...
  for (std::size_t index = 0U; index < joint_names.size(); ++index) {
    ...
    joint->set_raw_target_position_rad(
        output.raw_joint_target.positions.at(index));
    joint->set_raw_target_velocity_radps(
        output.raw_joint_target.velocities.at(index));
    ...
    joint->set_velocity_utilization(std::abs(output.state.velocities[index]) /
                                    limits.max_velocity.at(index));
    joint->set_acceleration_utilization(
        std::abs(output.state.accelerations[index]) /
        limits.max_acceleration.at(index));
    joint->set_jerk_utilization(std::abs(output.state.jerks[index]) /
                                limits.max_jerk.at(index));
...
proto::PlannerTelemetry makePlannerTelemetry(
    ...
    const mcc::PlanningDiagnostics &diagnostics, double sample_time_s) {
...
proto::CollisionTelemetry
makeCollisionTelemetry(const proto::SampleContext &context,
  ...
  *message.mutable_context() = context;
  ...
  for (const auto &source : collision.pairs) {
    ...
    pair->set_margin_shortfall_m(
        std::max(0.0, collision.minimum_distance_m - source.distance_after_m));
...
std::string elbowTeleopEventDetail(
    const mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::ElbowTeleopEvent &event,
...
std::string traceProjectionEvents(
    const mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::ProjectionDiagnostics
        &diagnostics,
    const std::vector<std::string> &joint_names) {
  std::ostringstream output;
  for (std::size_t index = 0U; index < diagnostics.events.size(); ++index) {
    ...
    output << joint_names.at(event.joint_index) << ':'
           << mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::
...
void addLink4Targets(const SolverHandles &handles,
                     const Link4TargetSnapshot &target,
                     SolverRequest &request) {
  request.position_targets[2].handle = handles.red_left_link4;
  request.position_targets[2].position = target.left;
  request.position_targets[2].enabled = target.left_enabled;
  request.position_targets[3].handle = handles.red_right_link4;
  request.position_targets[3].position = target.right;
  request.position_targets[3].enabled = target.right_enabled;
}

void initializeRedRequest(const SolverHandles &handles,
                          const mcc::FrameName &reference_frame_name,
                          const Link4TargetSnapshot &link4_target,
                          SolverRequest &request) {
  request.reference_frame_name = reference_frame_name;
  request.position_targets.resize(4);
  request.orientation_targets.resize(2);
  request.position_targets[0].handle = handles.red.left_position;
  request.position_targets[1].handle = handles.red.right_position;
  request.orientation_targets[0].handle = handles.red.left_orientation;
  request.orientation_targets[1].handle = handles.red.right_orientation;
    ...
    for (const auto &pass : diagnostics.hierarchy.passes) {
      ...
      if (!pass.constraint_violations.empty()) {
        ...
        output << ",largest={kind="
               ...
               << ",bound_source="
               << hierarchicalBoundSourceName(violation.bound_source)
               << ",source=\"" << violation.source_name << "\",component=\""
               << violation.component_name
...
void fillRedDiagnostics(const SolverHandles &handles,
  ...
  for (const auto &task : diagnostics.hierarchy.tasks) {
    ...
    } else if (task.handle_value == handles.red_yellow_posture.value) {
      output.yellow_posture_error_rad = task.target_error_norm;
    }
  }
  ...
  for (std::size_t index = 0; index < diagnostics.hierarchy.passes.size();
    ...
    output.pass_attempted[index] =
        diagnostics.hierarchy.passes[index].attempted;
    output.pass_succeeded[index] =
        diagnostics.hierarchy.passes[index].succeeded;
  }
  output.terminal_status = diagnostics.hierarchy.terminal_status;
}
...
void fillSelfCollisionDebug(
    ...
    const mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::SolverOptions &options,
...
std::array<double, 3> visualizationPoint(const Eigen::Vector3d &point) {
  return {point.x(), point.y(), point.z()};
}

void appendElbowSideScene(motion_control::viz::RenderBatch &batch,
                          const std::string &reference_frame,
                          std::string_view side, const Eigen::Vector3d &target,
                          bool target_enabled, const Eigen::Vector3d &raw,
                          const Eigen::Vector3d &executed) {
  namespace contract = contracts::mcl_nullspace_v1;
  namespace mcv = motion_control::viz;
  const std::string entity_prefix = std::string{side} + "_link4_";
  const double target_alpha = target_enabled ? 0.95 : 0.0;
  const double error_alpha = target_enabled ? 0.75 : 0.0;

  batch.spheres.push_back(mcv::Sphere3d{entity_prefix + "target",
                                        contract::kElbowSceneTopic,
                                        reference_frame,
                                        visualizationPoint(target),
                                        0.04,
                                        {0.15, 1.0, 0.2, target_alpha}});
  batch.spheres.push_back(mcv::Sphere3d{entity_prefix + "raw",
                                        contract::kElbowSceneTopic,
                                        reference_frame,
                                        visualizationPoint(raw),
                                        0.02,
                                        {0.15, 0.5, 1.0, 0.8}});
  batch.spheres.push_back(mcv::Sphere3d{entity_prefix + "executed",
                                        contract::kElbowSceneTopic,
                                        reference_frame,
                                        visualizationPoint(executed),
                                        0.03,
                                        {1.0, 0.5, 0.05, 0.75}});
  batch.line_strips.push_back(mcv::LineStrip3d{
      entity_prefix + "error",
      contract::kElbowSceneTopic,
      reference_frame,
      {visualizationPoint(target), visualizationPoint(executed)},
      {0.15, 1.0, 0.2, error_alpha},
      3.0,
      true});
}
...
void appendOtgExecution(motion_control::viz::RenderBatch &batch,
  ...
  batch.joint_states.push_back(motion_control::viz::JointStateSample{
      contract::kJointExecutionTopic, joint_names, positions, velocities});
  batch.poses.push_back(makeVisualizationPose(
      contract::kLeftCartesianExecutionTopic, reference_frame, left_pose));
  batch.poses.push_back(makeVisualizationPose(
      contract::kRightCartesianExecutionTopic, reference_frame, right_pose));
}
...
void appendNullspaceElbowScene(motion_control::viz::RenderBatch &batch,
  ...
  batch.spheres.reserve(batch.spheres.size() + 6U);
  batch.line_strips.reserve(batch.line_strips.size() + 2U);
  appendElbowSideScene(batch, reference_frame, "left", target.left,
                       target.left_enabled, raw_left_pose.translation(),
                       executed_left_pose.translation());
  appendElbowSideScene(batch, reference_frame, "right", target.right,
                       target.right_enabled, raw_right_pose.translation(),
                       executed_right_pose.translation());
}
...
int runLoop(Options planned_options, const R1RobotConfig &robot,
    ...
    if (!replay_options.output_dir_explicit) {
      ...
      const std::filesystem::path output_root =
          replay_options.output_root.value_or(std::filesystem::path{
              "runs/mcl_planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim"});
  ...
  Link4TargetSnapshot initial_link4_target;
  ...
  initial_output.raw_left_link4_pose =
      requirePose(initial_fk.poses, robot.left_link4_frame).pose;
  initial_output.raw_right_link4_pose =
      requirePose(initial_fk.poses, robot.right_link4_frame).pose;
  initial_output.left_pose = warmup_target.left;
  initial_output.right_pose = warmup_target.right;
  initial_output.left_link4_pose = initial_output.raw_left_link4_pose;
  initial_output.right_link4_pose = initial_output.raw_right_link4_pose;
  initial_output.link4_target = initial_link4_target;
  ...
  solver.beginRun(1);
  {
    ...
    SolverRequest red;
    initializeRedRequest(handles, robot.base_frame, initial_link4_target, red);
    ...
    fillRedDiagnostics(handles, diagnostics, initial_output);
  ...
  if (options.simulation.viewer_enabled) {
  ...
  const DragSnapshot initial_drag = initialDragSnapshot(initial_output, robot);
  mcl::LatestValueMailbox<DragSnapshot> drag_to_red(initial_drag);
  mcl::LatestValueMailbox<Link4TargetSnapshot> link4_target_to_red(
      initial_link4_target);
  ...
  link4_target_to_red.publish(initial_link4_target);
  ...
  ReplayPipelineGate replay_pipeline_gate;
  if (planned_options.source_mode == SourceMode::Replay &&
      replay_source->paused()) {
    replay_pipeline_gate.pause();
  }
  ...
  if (planned_options.replay_trace_enabled) {
    replay_trace
        << "attempt,source_revision,original_logical_timestamp_ns,"
           ...
           "raw_target_positions,raw_target_velocities,raw_target_"
           "accelerations,"
  ...
  workers.yellow = std::thread([&]() {
    ...
    if (telemetry_enabled) {
      telemetry_observer = [&](const mcl::WorkerIterationResult &result,
                               ...
                               const mcl::PeriodicWorkerStatistics
                                   &statistics) {
        ...
        if (deadline_missed && !deadline_miss_active) {
          yellow_telemetry->tryPush(TelemetryRecord::log(
              ...
              motion_control::viz::LogLevel::Warning, "avoidance-deadline-miss",
        ...
        [&](double, std::int64_t) {
          ...
          if (telemetry_enabled) {
            telemetry_context = makeSampleContext(
                telemetry_stamp,
                accepted ? proto::ACCEPTED : proto::FATAL_REJECTED, accepted,
                diagnostics.run_generation, 0U, diagnostics.attempt_revision,
                diagnostics.value_revision);
            yellow_telemetry->tryPush(telemetryRecord(
                telemetry_stamp,
                contracts::mcl_telemetry_v1::kAvoidanceSolverTopic,
                makeSolverTelemetry(telemetry_context, "avoidance", diagnostics,
                                    solver_debug, options.solver)));
            if (accepted) {
              yellow_telemetry->tryPush(telemetryRecord(
                  telemetry_stamp, contracts::mcl_telemetry_v1::kCollisionTopic,
  ...
  workers.red = std::thread([&]() {
    ...
    DragSnapshot drag = initial_drag;
    Link4TargetSnapshot link4_target = initial_link4_target;
    RedOutputSnapshot output = initial_output;
    SolverRequest request;
    initializeRedRequest(handles, robot.base_frame, link4_target, request);
    SolverSolution solution;
    SolverDiagnostics diagnostics;
    ...
    DualArmAdmittance admittance;
    admittance.configure(options.admittance);

    mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::JointTargetBuilder
        joint_target_builder(planned_options.joint_target,
                             1.0 / options.red_rate_hz, joint_names.size());
    mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::ReplaySettlingCounter
        replay_settling(planned_options.replay_settling);
    EventStamp telemetry_stamp;
    proto::SampleContext telemetry_context;
    mcl::PeriodicIterationObserver telemetry_observer;
    bool deadline_miss_active = false;
    ...
    const auto makeReplayRow = [&](std::uint64_t attempt_revision) {
      ...
      row[14] =
          mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::jointTargetModeName(
      ...
      const std::size_t source_index =
          std::min(*target.replay_source_index,
    ...
    mcl::runPeriodicWorker(
        ...
        [&](double, std::int64_t sample_time_ns) {
          ...
          if (link4_target_to_red.readLatest(link4_target)) {
          ...
          if (shouldFreezeControlPipeline(target.replay_joint_hold)) {
          ...
          if (!staged_planner_sample.has_value()) {
            ...
            if (!planning_status.ok()) {
              ...
              red_attempt_to_ui.publish(attempt);
              if (telemetry_enabled) {
                telemetry_context.set_outcome(proto::FATAL_REJECTED);
              }
              return mcl::WorkerIterationResult{
          ...
          const DualAdmittanceOutput compliance = admittance.step(
              staged_for_attempt, output.left_pose, output.right_pose,
              robot.left_tcp_offset, robot.right_tcp_offset, drag.references,
          ...
          reference.left = command_for_attempt.frames.at(0).pose;
          reference.right = command_for_attempt.frames.at(1).pose;
          ...
          if (telemetry_enabled) {
            telemetry_context = makeSampleContext(
                telemetry_stamp,
                ik_accepted ? proto::ACCEPTED : proto::FATAL_REJECTED, false,
                diagnostics.run_generation, target.revision,
                diagnostics.attempt_revision, diagnostics.value_revision);
          }
          if (!ik_accepted) {
            attempt.failure_disposition = postAdmittanceFailureDisposition();
            attempt.state = RedAttemptState::FatalRejected;
            attempt.target = target;
            ...
            if (telemetry_enabled) {
              ...
              red_telemetry->tryPush(telemetryRecord(
                  telemetry_stamp,
                  contracts::mcl_telemetry_v1::kCartesianComplianceTopic,
                  makeCartesianCompliance(telemetry_context, compliance)));
              red_telemetry->tryPush(telemetryRecord(
                  telemetry_stamp, contracts::mcl_telemetry_v1::kIkSolverTopic,
          ...
          const auto raw_right_pose =
              requirePose(solution.kinematics_solution.solved_poses,
                          robot.right_end_effector_frame)
                  .pose;
          const auto raw_left_link4_pose =
              requirePose(solution.kinematics_solution.solved_poses,
                          robot.left_link4_frame)
                  .pose;
          const auto raw_right_link4_pose =
              requirePose(solution.kinematics_solution.solved_poses,
          ...
          const auto mapped_raw_ik =
              mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::mapActiveIkToFull(
          ...
          mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::ProjectionDiagnostics
              projection;
          auto projected_target = mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::
              projectConfiguredLimits(raw_target, joint_otg_limits, projection);
          ...
          if (!joint_status.ok()) {
            ...
            if (telemetry_enabled) {
              ...
              red_telemetry->tryPush(telemetryRecord(
                  telemetry_stamp,
                  contracts::mcl_telemetry_v1::kCartesianTrackingTopic,
                  makeCartesianTracking(telemetry_context, target, reference,
                                        command_for_attempt, raw_left_pose,
                                        raw_right_pose, output.left_pose,
                                        output.right_pose)));
            }
            return mcl::WorkerIterationResult{
                mcl::WorkerIterationOutcome::FatalRejected,
          ...
          const auto executed_right_pose =
              requirePose(executed_fk.poses, robot.right_end_effector_frame)
                  .pose;
          const auto executed_left_link4_pose =
              requirePose(executed_fk.poses, robot.left_link4_frame).pose;
          const auto executed_right_link4_pose =
              requirePose(executed_fk.poses, robot.right_link4_frame).pose;
          ...
          output.raw_left_link4_pose = raw_left_link4_pose;
          output.raw_right_link4_pose = raw_right_link4_pose;
          output.left_pose = executed_left_pose;
          output.right_pose = executed_right_pose;
          output.left_link4_pose = executed_left_link4_pose;
          output.right_link4_pose = executed_right_link4_pose;
          output.link4_target = link4_target;
          output.admittance = compliance;
          ...
          fillRedDiagnostics(handles, diagnostics, output);
          ...
          if (telemetry_enabled) {
            ...
            red_telemetry->tryPush(telemetryRecord(
                ...
                makeCartesianTracking(telemetry_context, target, reference,
                                      command_for_attempt, raw_left_pose,
                                      raw_right_pose, executed_left_pose,
                                      executed_right_pose)));
            red_telemetry->tryPush(telemetryRecord(
                telemetry_stamp,
  ...
  std::uint64_t published_link4_revision = initial_link4_target.revision;
  ...
  if (planned_options.source_mode == SourceMode::Replay &&
    ...
    latest_input_targets = {{mcl::ArmSide::Left, source.value.left.pose},
  ...
  std::uint64_t simulated_state_sequence = initial_state.sequence;
  ...
  updateQpPassTimePercentiles(frame.solvers[0], red_qp_pass_time_percentiles);
  ...
  while (true) {
    ...
    if (latest_output.state.sequence != simulated_state_sequence) {
      simulation.setKinematicState(
          simulationJointState(latest_output.state, joint_names));
      simulation.forward();
      simulated_state_sequence = latest_output.state.sequence;
    }
    if (viewer) {
      if (viewer->shouldClose()) {
        break;
      }
      const auto interactions = viewer->pollInteractions();
      if (interactions.size() != 2U) {
        throw std::runtime_error(
            "MuJoCo viewer returned an unexpected handle count");
      }
      DragSnapshot drag;
      drag.references[0] = interactions[0];
      drag.references[1] = interactions[1];
      drag_to_red.publish(drag);
    }
    ...
    input.setExecutedLink4Positions(
    ...
    if (!held_fault.has_value()) {
      ...
      if (input.link4Targets().revision != published_link4_revision) {
        ...
        link4_target_to_red.publish(input.link4Targets());
      }
    }
      ...
      if (advance.frame_changed) {
        ...
        if (pending_step_start_ns.has_value()) {
          const std::int64_t frame_duration_ns =
              ...
              *pending_step_start_ns;
          replay_pipeline_gate.grantSingleFrame(
              workerTicksForReplayDuration(frame_duration_ns,
                                           options.red_rate_hz),
              workerTicksForReplayDuration(frame_duration_ns,
                                           options.yellow_rate_hz));
        }
    ...
    if (schedule->update_due) {
      ...
      frame.status +=
          " | JointPlanner " +
          std::string{mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::
                          planningSynchronizationName(
                              planned_options.planning.joint_synchronization)} +
      ...
      const auto makePlannerArm = [&](mcl::ArmSide side, std::size_t index) {
        ...
        const auto &reference = planner_frame.pose;
        const auto &fk =
      ...
      EventStamp emit_stamp{schedule->emit_time_ns,
                            static_cast<std::uint64_t>(std::max<std::int64_t>(
                                0, schedule->sample_time_ns)),
                            0U};
      if (telemetry_enabled) {
      ...
      mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::appendNullspaceElbowScene(
          ...
          latest_output.raw_left_link4_pose, latest_output.raw_right_link4_pose,
      ...
      if (telemetry_enabled) {
        ...
        const auto attempt_timestamp =
            latest_red_attempt.event_timestamp_ns == 0U
                ? emit_stamp.timestamp_ns
        ...
        for (auto &pose : visualization_frame.poses) {
          if (pose.channel == contracts::mcl_state_v1::kLeftCartesianIkTopic ||
              ...
              pose.channel ==
                  contracts::mcl_execution_v1::kLeftCartesianExecutionTopic ||
              pose.channel ==
        ...
        if (run_info_due) {
          ...
          *run_info.mutable_timestamp() = info_context.timestamp();
          ...
          run_info.set_schema_version(
              contracts::mcl_telemetry_v1::kSchemaVersion);
          ...
          add_option("mujoco_model", options.simulation.mujoco_model_path);
          add_option("mujoco_viewer",
          ...
          add_option("cartesian_synchronization",
                     planningSynchronizationName(
                         planned_options.planning.cartesian_synchronization));
          add_option("joint_synchronization",
                     planningSynchronizationName(
                         planned_options.planning.joint_synchronization));
          ...
          add_option("max_angular_velocity_radps",
                     std::to_string(
                         planned_options.planning.max_angular_velocity_rps));
          add_option(
          ...
          telemetry_encoder->appendMessage(
              contracts::mcl_telemetry_v1::kRunInfoTopic, run_info, emit_stamp,
              emit_stamp, visualization_frame, encode_statistics);
        ...
        if (loaded_replay.has_value()) {
          ...
          *replay_message.mutable_context() = makeSampleContext(
          ...
          replay_message.set_playback_rate(
              planned_options.replay->playback_rate);
          replay_message.set_paused(replay_source->paused());
          replay_message.set_end_of_stream(replay_source->endOfStream() ||
                                           will_finish_replay);
          ...
          replay_message.set_joint_acceleration_within_tolerance(
              acceleration_within);
          replay_message.set_settled_cycles(replay_settled_cycle_count.load());
          ...
          telemetry_encoder->appendMessage(
              ...
              emit_stamp, emit_stamp, visualization_frame, encode_statistics);
        ...
        *transport.mutable_context() = makeSampleContext(
            ...
            latest_red_attempt.solver_debug.grouped_attempt.has_value()
                ...
                : 0U,
            latest_output.revision);
        transport.set_sink_kind(options.visualization.mcap_path.has_value()
                                    ? "foxglove_websocket+mcap"
                                    : "foxglove_websocket");
        ...
        telemetry_encoder->appendMessage(
            contracts::mcl_telemetry_v1::kTransportTopic, transport, emit_stamp,
            emit_stamp, visualization_frame, encode_statistics);
      }
    ...
    if (schedule->draw_due) {
      ...
      nullspace_debug.executed_left_link4 =
          latest_output.left_link4_pose.translation();
      nullspace_debug.executed_right_link4 =
          latest_output.right_link4_pose.translation();
      nullspace_debug.left_link4_raw_error_m =
          (nullspace_debug.link4_target.left - nullspace_debug.raw_left_link4)
              .norm();
      nullspace_debug.right_link4_raw_error_m =
          (nullspace_debug.link4_target.right - nullspace_debug.raw_right_link4)
              .norm();
      nullspace_debug.left_link4_executed_error_m =
          (nullspace_debug.link4_target.left -
           nullspace_debug.executed_left_link4)
              .norm();
      nullspace_debug.right_link4_executed_error_m =
          (nullspace_debug.link4_target.right -
           nullspace_debug.executed_right_link4)
              .norm();
      nullspace_debug.left_tcp_position_error_m =
          latest_output.left_position_error_m;
      nullspace_debug.right_tcp_position_error_m =
          latest_output.right_position_error_m;
      nullspace_debug.left_tcp_orientation_error_rad =
          latest_output.left_orientation_error_rad;
      ...
      if (latest_output.fallback_priority.has_value()) {
        nullspace_debug.fallback_priority =
            priorityName(*latest_output.fallback_priority);
      }
      nullspace_debug.primary_attempted = latest_output.pass_attempted[0];
      ...
      mcl::PlannedGroupedTuiSnapshot tui_snapshot;
      tui_snapshot.frame = &frame;
      tui_snapshot.presentation = &presentation;
      tui_snapshot.joint_otg = tui_debug;
      tui_snapshot.publish_count = publish_count;
      tui_snapshot.sink_status = visualization_sink->status();
      tui_snapshot.title = kTitle;
      tui_snapshot.input_status = input.status();
      tui_snapshot.extra_pages = {makeNullspaceTuiPage(nullspace_debug)};
      tui_snapshot.header_context = input.headerContext();
      tui_snapshot.footer_hints = input.footerHints();
      tui_snapshot.help_lines = input.helpLines();
      tui.render(tui_snapshot);
    }
    if (viewer) {
      viewer->render(viewerWrenchArrows(latest_output.admittance));
    }
    if (planned_options.replay.has_value() &&
  ...
  if (planned_options.replay.has_value()) {
    ...
    execution.topology =
        "planned-red-yellow-hierarchical-step+cartesian-admittance+"
        "split-reference-joint-otg+mujoco-kinematic-forward";
    execution.solver =
        "motion_control_core+CartesianPlanner+CartesianAdmittance+JointPlanner";
    execution.backend = "proxqp+ruckig+mujoco-forward";
    ...
    execution.resolved_config = {
        ...
        {"joint_target_mode",
         mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::jointTargetModeName(
             planned_options.joint_target.mode)},
        {"simulation_mode", "mujoco-kinematic-forward"},
        {"mujoco_model", options.simulation.mujoco_model_path},
        {"mujoco_viewer", options.simulation.viewer_enabled ? "on" : "off"},
        {"angular_admittance",
         options.admittance.angular_enabled ? "on" : "off"},
    ...
    joint_otg["algorithm"] = mcl::planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::
        jointPlanningAlgorithmName(planned_options.planning.joint_algorithm);
    joint_otg["synchronization"] = mcl::
        planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim::planningSynchronizationName(
            planned_options.planning.joint_synchronization);
    ...
    joint_otg["profile_source_revision"] =
        options.robot.joint_stream.source_revision;
    joint_otg["profile_source_path"] = options.robot.joint_stream.source_path;
