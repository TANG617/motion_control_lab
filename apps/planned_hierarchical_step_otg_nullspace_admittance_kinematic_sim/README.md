...
`mcl_planned_hierarchical_step_otg_nullspace` 的独立导纳/MuJoCo 运动学变体。它完整保留
Cartesian OTG、两级 HKS、link4/elbow null-space、JointPlanner OTG、teleop、replay、TUI、
遥测和 artifact；只在 `CartesianPlanner::step()` 与 HKS 之间增加左右独立的
`CartesianAdmittance`。
...
```bash
/workspace/install/algorithm/bin/mcl_planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim \
  teleop \
...
```bash
MCL_MUJOCO_VIEWER=off MCL_UI=none MCL_VIZ=none \
  apps/planned_hierarchical_step_otg_nullspace_admittance_kinematic_sim/scripts/run_keyboard.sh \
  --duration 0.25
```
