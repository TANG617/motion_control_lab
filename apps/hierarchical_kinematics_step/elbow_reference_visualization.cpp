...
const auto &schema() {
  static const auto data = [] {
    Json::Value value;
    value["type"] = "object";
    value["title"] = "Red-consumed left posture reference angle";
    for (const char *key :
    ...
    for (const char *key :
         {"secondary_attempted", "secondary_succeeded", "secondary_selected", "mirror_tcp_input"})
...
double ElbowReferenceAngleTracker::update(const ElbowPrediction &p) {
  const double angle = std::atan2(p.sine, p.cosine);
...
ElbowReferenceVisualizationSnapshot makeElbowReferenceVisualizationSnapshot(
  ...
  result.angle_unwrapped_rad = tracker.update(consumed.prediction);
  result.shoulder = shoulder;
  result.wrist = ee_reference * geometry.wrist_in_ee;
  result.circle = geometry.referenceCircle(shoulder, ee_reference);
  return result;
}
...
void appendElbowReferenceVisualization(motion_control::viz::RenderBatch &batch,
  ...
  const double phi = std::atan2(p.sine, p.cosine);
  Json::Value message;
  ...
  message["cos_phi"] = p.cosine;
  message["sin_phi"] = p.sine;
  message["angle_rad"] = phi;
  message["angle_deg"] = phi * kDegrees;
  message["angle_unwrapped_deg"] = s.angle_unwrapped_rad * kDegrees;
  ...
  batch.encoded_messages.push_back(
      {source == "harp" ? "/mcl/harp/left/angle" : kRecordedElbowAngleTopic, "json", source == "harp" ? "mcl.harp.Angle" : "mcl.elbow_reference.Angle", "jsonschema", schema(),
  ...
  const auto add = [&](const char *id, const mcv::ColorRgba &color,
    ...
    batch.line_strips.push_back(
        {id, source == "harp" ? "/mcl/harp/left/scene" : kRecordedElbowSceneTopic, frame, std::move(points), color, width, true});
  };
  const auto &circle = s.circle;
  ...
  add("shoulder_wrist_axis", gray, {point(s.shoulder), point(s.wrist)}, 2);
  ...
  add(source == "harp" ? "harp_direction" : "elbow_reference_direction", green, {point(circle.center), record.target}, 3);
