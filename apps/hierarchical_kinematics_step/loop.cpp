...
std::string elbowTeleopEventDetail(
    const mcl::hierarchical_kinematics_step::ElbowTeleopEvent &event,
...
mcl::R1RobotConfig presentationRobotConfig(const RobotOptions &robot) {
  return mcl::R1RobotConfig{
      ...
      robot.left_tcp_offset,
      robot.right_tcp_offset,
      robot.joint_names,
...
void fillSelfCollisionDebug(
    ...
    const mcl::hierarchical_kinematics_step::SolverOptions &options,
...
int runLoop(Options planned_options, const R1RobotConfig &robot,
  ...
  std::unique_ptr<mcs::MujocoKinematicSimulation> simulation;
  ...
  const auto presentation_robot = presentationRobotConfig(robot);
  auto presentation = mcl::makeArmPresentation(
      presentation_robot, mcl::foxgloveIkVisualizationChannels());
  ...
  const bool telemetry_enabled =
      capabilities.telemetry && options.visualization.enabled;
  ...
  workers.red = std::thread([&]() {
    ...
    std::unique_ptr<mcl::hierarchical_kinematics_step::JointTargetBuilder>
        joint_target_builder;
    if (capabilities.joint_otg) {
      joint_target_builder = std::make_unique<
          mcl::hierarchical_kinematics_step::JointTargetBuilder>(
          planned_options.joint_target, 1.0 / options.red_rate_hz,
          joint_names.size());
    }
    std::unique_ptr<mcl::hierarchical_kinematics_step::ReplaySettlingCounter>
        replay_settling;
    if (capabilities.joint_otg) {
      replay_settling = std::make_unique<
          mcl::hierarchical_kinematics_step::ReplaySettlingCounter>(
    ...
    mcl::runPeriodicWorker(
        ...
        [&](double, std::int64_t sample_time_ns) {
          ...
          if (!capabilities.cartesian_planning) {
            auto direct_sample = accepted_planner_sample;
            direct_sample.frames.at(0).pose = target.left;
            direct_sample.frames.at(0).twist.setZero();
            direct_sample.frames.at(0).acceleration.setZero();
            direct_sample.frames.at(1).pose = target.right;
            direct_sample.frames.at(1).twist.setZero();
            direct_sample.frames.at(1).acceleration.setZero();
            direct_sample.time_from_start = 0.0;
          ...
          const auto mapped_raw_ik =
              mcl::hierarchical_kinematics_step::mapActiveIkToFull(
          ...
          mcl::hierarchical_kinematics_step::JointTarget raw_target;
          if (capabilities.joint_otg) {
            raw_target = joint_target_builder->preview(
                mapped_raw_ik.positions, mapped_raw_ik.velocities);
          } else {
            raw_target.positions = mapped_raw_ik.positions;
          ...
          joint_sample.positions = projected_target.positions;
          ...
          mcc::PlanningDiagnostics joint_plan_diagnostics;
          mcc::PlanningDiagnostics joint_step_diagnostics;
          joint_plan_diagnostics.state = mcc::PlanningState::Finished;
          joint_step_diagnostics.state = mcc::PlanningState::Finished;
          if (capabilities.joint_otg) {
          ...
          if (loaded_replay.has_value()) {
            ...
            replay_settled_cycle_count.store(
                replay_settling ? replay_settling->consecutiveCycles() : 0U);
  ...
  while (true) {
    ...
    if (simulation &&
        latest_output.state.sequence != simulated_state_sequence) {
      simulation->setKinematicState(
      ...
      if (telemetry_enabled) {
        ...
        if (run_info_due) {
          ...
          *run_info.mutable_timestamp() = info_context.timestamp();
          ...
          add_option("profile", profileName(planned_options.profile));
          add_option("resolved_options_json",
                     resolvedOptionsJson(planned_options));
  ...
  if (planned_options.replay.has_value()) {
    ...
    if (capabilities.cartesian_planning)
      execution.solver += "+CartesianPlanner";
    if (capabilities.admittance)
      execution.solver += "+CartesianAdmittance";
    if (capabilities.joint_otg)
      execution.solver += "+JointPlanner";
    ...
    execution.original_argv = planned_options.binary_argv;
    execution.launcher = planned_options.replay->launcher;
    ...
    Json::Value complete_resolved_options;
    Json::CharReaderBuilder json_reader;
    std::string json_errors;
    std::istringstream resolved_input{resolvedOptionsJson(planned_options)};
    if (!Json::parseFromStream(json_reader, resolved_input,
                               &complete_resolved_options, &json_errors)) {
      throw std::runtime_error("failed to serialize resolved options: " +
                               json_errors);
    }
