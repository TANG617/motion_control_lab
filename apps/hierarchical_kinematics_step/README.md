...
- `planned`：CartesianTrajectoryGenerator -> legacy HKS；
- `planned-otg`：CartesianTrajectoryGenerator -> legacy HKS -> JointPtpTrajectoryGenerator；
- `planned-otg-nullspace`：双臂 position Primary > soft orientation/posture/link4 Secondary HKS + JointPtpTrajectoryGenerator；
...
TCP goal -> nominal EE Cartesian OTG P/V/A
         ...
         -> HKS pose/twist -> joint target projection -> JointPtpTrajectoryGenerator OTG
...
pause/start-paused 会冻结 Cartesian planner、导纳、HKS 和 JointPtpTrajectoryGenerator 整条计算链，`.` 单步让
整条链共同推进。导纳推进后的 HKS、JointPtpTrajectoryGenerator 或 executed FK 失败为 fatal；导纳之前的
...
规划 profile 的公开请求通过原有 `runLoop` 的 CartesianTrajectoryGenerator/JointPtpTrajectoryGenerator；JSON 由 app 将 frame/TCP 明确转换到物理 TCP CSV，并以公开 `--initial-state` 保留输入 q/v。原 CSV/MCAP `replay` 继续直接读取。
