...
std::vector<unsigned int> normalizedCpuList(
  ...
  std::size_t cpu_count)
{
  ...
  if (result.empty()) {
    throw std::runtime_error("CPU affinity request must not be empty");
  }
  return result;
}
...
{
  return
    "app=" + app + " role=" + role + " requested=" + cpuList(requested) +
  ...
  std::size_t cpu_count) const
{
  ...
  if (!std::includes(
      allowed_cpus_.begin(), allowed_cpus_.end(),
      requested_cpus.begin(), requested_cpus.end()))
  {
  ...
  std::size_t cpu_count) const
{
  ...
  for (const auto cpu : binding.requested_cpus) {
    if (cpu >= CPU_SETSIZE) {
      throw std::runtime_error(
              ...
              bindingContext(
                app, role, binding.requested_cpus, allowed_cpus_, effective_before));
    }
    CPU_SET(cpu, &requested_mask);
  }

  const int error =
    pthread_setaffinity_np(pthread_self(), sizeof(requested_mask), &requested_mask);
  if (error != 0) {
    throw std::runtime_error(
  ...
  if (binding.effective_cpus != binding.requested_cpus) {
    throw std::runtime_error(
            ...
            bindingContext(
              app, role, binding.requested_cpus, allowed_cpus_, binding.effective_cpus));
  }
  binding.thread_id = currentThreadId();

  std::ostringstream diagnostic;
  diagnostic << "cpu_affinity app=" << app << " role=" << role
