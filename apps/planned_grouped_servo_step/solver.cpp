...
void addRedAccelerationLimits(mcc::KinematicsSolverBuilder &builder,
  ...
  acceleration.enforcement =
      mcc::HardEnforcement{options.maximum_accepted_hard_violation};
  mcc::GroupedJointAccelerationLimitHandle handle;
  requireOk(builder.addJointAccelerationLimits(mcc::SolverGroup::Red,
                                               acceleration, handle),
...
mcc::GroupedKinematicsSolverConfig makeSolverConfig(const Options &options) {
  ...
  config.red.qp.proxqp.primal_infeasibility_tolerance =
...
void configureSolver(
  ...
  const auto &solver = options.interactive.solver;
  handles.red =
  ...
  addRedAccelerationLimits(builder, solver);
