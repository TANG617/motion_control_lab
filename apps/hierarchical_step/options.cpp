...
void printHierarchicalUsage(const char *program) {
  ...
  std::cout
      ...
      << "  --cartesian-progress-weight <value>      Scaled Cartesian progress "
         "weight\n"
...
HierarchicalOptions parseHierarchicalOptions(int argc, char **argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    } else if (optionIn(argument, {"--regularization", "--position-tolerance-m",
...
LaunchOptions parseOptions(int argc, char **argv) {
  ...
  if (mode == "teleop") {
    ...
    result.interactive = parseHierarchicalOptions(argc - 1, argv + 1);
    return result;
  }
  if (mode != "replay") {
  ...
  for (int index = 2; index < argc; ++index) {
    ...
    const bool hierarchical_value = optionIn(
        ...
        {"--red-rate", "--yellow-rate", "--ui-rate", "--deadline-policy",
         ...
         "--red-proxqp-absolute-tolerance", "--yellow-posture-weight",
