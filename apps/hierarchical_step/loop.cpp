...
namespace mcc_diagnostics = motion_control::core;

const char *
resultDispositionName(mcc_diagnostics::ResultDisposition value) {
  return mcc_diagnostics::isAccepted(value) ? "accepted" : "rejected";
}

const char *
jointLimitPolicyName(mcc_diagnostics::KinematicsJointLimitPolicy value) {
  switch (value) {
  case mcc_diagnostics::KinematicsJointLimitPolicy::ModelPositionOnly:
    return "model-position";
  case mcc_diagnostics::KinematicsJointLimitPolicy::
      ModelPositionAndVelocity:
    return "model-position+velocity";
  case mcc_diagnostics::KinematicsJointLimitPolicy::ExplicitRequirements:
    return "explicit-requirements";
  case mcc_diagnostics::KinematicsJointLimitPolicy::Unconstrained:
...
const char *
terminationReasonName(mcc_diagnostics::IkTerminationReason value) {
  switch (value) {
  case mcc_diagnostics::IkTerminationReason::NotStarted:
    return "not-started";
  case mcc_diagnostics::IkTerminationReason::Converged:
    return "converged";
  case mcc_diagnostics::IkTerminationReason::NoEnabledConvergenceTasks:
    return "no-convergence-tasks";
  case mcc_diagnostics::IkTerminationReason::SingleIteration:
    return "single-iteration";
  case mcc_diagnostics::IkTerminationReason::Saturated:
    return "saturated";
  case mcc_diagnostics::IkTerminationReason::NoProgress:
    return "no-progress";
  case mcc_diagnostics::IkTerminationReason::IterationBudget:
    return "iteration-budget";
  case mcc_diagnostics::IkTerminationReason::SoftTimeBudget:
    return "soft-time-budget";
  case mcc_diagnostics::IkTerminationReason::HardConstraintViolation:
    return "hard-constraint-violation";
  case mcc_diagnostics::IkTerminationReason::InvalidNumericalSolution:
    return "invalid-numerical-solution";
  }
  return "unknown";
}

const char *qpBackendName(mcc_diagnostics::QpBackend value) {
  return value == mcc_diagnostics::QpBackend::ProxQp ? "proxqp"
                                                         : "eiquadprog";
}

const char *qpStatusName(mcc_diagnostics::QpSolveStatus value) {
  switch (value) {
  case mcc_diagnostics::QpSolveStatus::Optimal:
    return "optimal";
  case mcc_diagnostics::QpSolveStatus::PrimalInfeasible:
    return "primal-infeasible";
  case mcc_diagnostics::QpSolveStatus::DualInfeasible:
    return "dual-infeasible";
  case mcc_diagnostics::QpSolveStatus::MaximumIterations:
    return "maximum-iterations";
  case mcc_diagnostics::QpSolveStatus::NumericalFailure:
    return "numerical-failure";
  case mcc_diagnostics::QpSolveStatus::NotRun:
    return "not-run";
  }
  return "unknown";
}
...
void updateLocalSolverDebug(
    SolverDebug &output,
    const mcc_diagnostics::InverseKinematicsDiagnostics &diagnostics,
    mcc_diagnostics::ResultDisposition disposition) {
...
const char *
hierarchicalRejectionReasonName(hierarchical_step::SolverRejectionReason value) {
  switch (value) {
  case hierarchical_step::SolverRejectionReason::None:
    return "none";
  case hierarchical_step::SolverRejectionReason::InvalidTarget:
    return "invalid-target";
  case hierarchical_step::SolverRejectionReason::SolverRejected:
    return "solver-rejected";
  }
  return "unknown";
}

const char *couplingStateName(hierarchical_step::CouplingState value) {
  switch (value) {
  case hierarchical_step::CouplingState::Unavailable:
    return "unavailable";
  case hierarchical_step::CouplingState::WaitingForValue:
    return "waiting-for-value";
  case hierarchical_step::CouplingState::Active:
    return "active";
  case hierarchical_step::CouplingState::RejectedSource:
...
void updateSolverDebug(SolverDebug &output,
                       const hierarchical_step::SolverDiagnostics &diagnostics,
...
void updateSolverDebug(SolverDebug &output,
  ...
  if (diagnostics.hierarchical) {
    ...
    for (std::size_t index = 0;
      ...
      destination.name = source.name;
      destination.active = source.active;
      destination.scale = source.weighted_progress_scale;
      destination.cost = 0.0;
      destination.degraded = source.degraded;
      destination.stuck = source.stuck;
    }
...
using mcl::hierarchical_step::CartesianHandles;
using mcl::hierarchical_step::LaunchOptions;
using mcl::hierarchical_step::requireOk;
using mcl::hierarchical_step::SolverDiagnostics;
using mcl::hierarchical_step::SolverHandles;
using mcl::hierarchical_step::SolverRequest;
using mcl::hierarchical_step::SolverRuntime;
using mcl::hierarchical_step::SolverSolution;
using mcl::hierarchical_step::SourceMode;

using mcl::toEigen;
using mcl::toStdVector;

constexpr const char *kProgramId = "mcl_hierarchical_step";
constexpr const char *kTitle = "Motion Control Hierarchical Dual-arm IK";
...
mcl::hierarchical_step::CapturedRobotState
capturedState(const StateSnapshot &state) {
...
void addCartesianTargets(const CartesianHandles &handles,
                         const TargetSnapshot &target, SolverRequest &request) {
...
void fillSelfCollisionDebug(
    ...
    const mcl::hierarchical_step::SolverOptions &options,
...
int runLoopImpl(LaunchOptions launch, const mcl::R1RobotConfig &robot,
    ...
    if (!launch.replay->output_dir_explicit) {
      ...
      const std::filesystem::path output_root =
          launch.replay->output_root.value_or(
              std::filesystem::path{"runs/mcl_hierarchical_step"});
    ...
    mcl::runPeriodicWorker(
        ...
        [&](double, std::int64_t sample_time_ns) {
          ...
          if (loaded_replay.has_value()) {
            ...
            replay_trace << diagnostics.attempt_revision << ','
                         << source.sequence << ','
  ...
  if (launch.replay.has_value()) {
    ...
    execution.topology = "red-yellow-hierarchical-step";
