...
bool isFrozenAlgorithmOption(const std::string &argument) {
  return argument == "--rate" || argument == "--solver" ||
         argument == "--backend" || argument == "--regularization" ||
         argument == "--maximum-iterations" ||
         argument == "--position-tolerance-m" ||
         argument == "--orientation-tolerance-rad" ||
         argument == "--posture-weight" ||
         argument == "--joint-position-margin-rad" ||
         argument == "--task-gain" || argument == "--task-weight";
}
...
[[noreturn]] void rejectFrozenOverride(const std::string &argument) {
  ...
  throw std::runtime_error(argument +
                           " is not supported; the baseline algorithm "
                           "configuration is production-static");
}
...
void printTeleopUsage(const char *program) {
  ...
  std::cout
      ...
      << "  --viz <foxglove|none>      Visualization transport (default: "
         "foxglove)\n"
