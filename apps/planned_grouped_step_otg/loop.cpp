...
const char *groupedRejectionReasonName(mcc::GroupedRejectionReason value) {
  switch (value) {
  case mcc::GroupedRejectionReason::None:
    return "none";
  case mcc::GroupedRejectionReason::SolverNotInitialized:
    return "solver-not-initialized";
  case mcc::GroupedRejectionReason::GroupUnavailable:
    return "group-unavailable";
  case mcc::GroupedRejectionReason::RunNotStarted:
    return "run-not-started";
  case mcc::GroupedRejectionReason::ConcurrentCall:
    return "concurrent-call";
  case mcc::GroupedRejectionReason::InvalidCapturedState:
    return "invalid-captured-state";
  case mcc::GroupedRejectionReason::InvalidTarget:
    return "invalid-target";
  case mcc::GroupedRejectionReason::SolverRejected:
    return "solver-rejected";
  case mcc::GroupedRejectionReason::RevisionOverflow:
...
const char *couplingStateName(mcc::GroupCouplingState value) {
  switch (value) {
  case mcc::GroupCouplingState::Unavailable:
    return "unavailable";
  case mcc::GroupCouplingState::WaitingForValue:
    return "waiting-for-value";
  case mcc::GroupCouplingState::Active:
    return "active";
  case mcc::GroupCouplingState::RejectedSource:
...
const char *compactJointName(std::size_t index) {
  constexpr std::array<const char *,
                       mcl::planned_grouped_step_otg::kR1JointCount>
...
const char *
projectionFlag(mcl::planned_grouped_step_otg::ProjectionComponent component) {
...
std::string traceProjectionEvents(
  ...
  for (std::size_t index = 0U; index < diagnostics.events.size(); ++index) {
    ...
    output << joint_names.at(event.joint_index) << ':'
           << mcl::planned_grouped_step_otg::projectionComponentName(
                  event.component)
...
void appendPlanningRequestPoses(motion_control::viz::RenderBatch &frame,
  ...
  namespace contract = contracts::foxglove_planned_grouped_step_otg_v1;
...
int runLoop(Options planned_options, const R1RobotConfig &robot,
        ...
        [&](double, std::int64_t sample_time_ns) {
          ...
          if (target.revision != planned_goal_revision) {
            ...
            attempt.retarget_clamp =
                mcl::planned_grouped_step_otg::clampRetargetCurrentState(
                    retarget_request);
          ...
          const auto ik_status = solver.solveInverseKinematics(
              mcc::SolverGroup::Red, request, solution, diagnostics);
          red_solve_time_percentiles.record(
              diagnostics.kinematics.solve_time_ms);
          const bool ik_accepted =
              operationSucceeded(ik_status) && diagnostics.attempt_accepted;
          if (!ik_accepted) {
            attempt.state =
                planned_options.source_mode == SourceMode::Teleop &&
          ...
          const auto mapped_raw_ik =
              mcl::planned_grouped_step_otg::mapActiveIkToFull(
          ...
          try {
            projected_target =
                mcl::planned_grouped_step_otg::projectConfiguredLimits(
                    raw_target, joint_otg_limits, projection);
          } catch (const std::exception &error) {
            attempt.state = RedAttemptState::FatalRejected;
            attempt.target = target;
            attempt.detail =
                std::string{"joint target projection failed: "} + error.what();
          ...
          const auto fk_status = solver.computeForwardKinematics(
              mcc::SolverGroup::Red, executed_fk_request, executed_fk,
              executed_fk_diagnostics);
          if (!fk_status.ok()) {
            attempt.state = RedAttemptState::FatalRejected;
            attempt.target = target;
            attempt.detail =
  ...
  tui_debug.source_mode = planned_options.source_mode == SourceMode::Replay
                              ? "mcap replay"
                              : "keyboard teleop";
  tui_debug.target_mode = mcl::planned_grouped_step_otg::jointTargetModeName(
      planned_options.joint_target_mode);
  ...
  if (planned_options.replay.has_value()) {
    ...
    execution.resolved_config = {
        ...
        {"joint_target_mode",
         mcl::planned_grouped_step_otg::jointTargetModeName(
             planned_options.joint_target_mode)},
    };
    auto manifest =
    ...
    for (std::size_t index = 0U; index < joint_names.size(); ++index) {
      ...
      joint_otg["max_acceleration_rad_per_s2"].append(
          mcl::planned_grouped_step_otg::kR1StreamMaxAccelerationRadPerS2
              [index]);
