...
JointGroupMaximum jointGroupMaximum(
  ...
  std::size_t end)
{
  ...
  for (std::size_t index = begin; index < std::min(end, joints.size()); ++index) {
    ...
    result.velocity = std::max(result.velocity, std::abs(joint.execution_velocity));
    result.acceleration = std::max(result.acceleration, std::abs(joint.execution_acceleration));
    result.jerk = std::max(result.jerk, std::abs(joint.execution_jerk));
    result.velocity_utilization = std::max(
    ...
    result.jerk_utilization = std::max(
...
TuiPage makeJointPlanningPage(const PlannedGroupedStepOtgTuiDebug & app)
{
  TuiPage page;
  ...
  page.sections.push_back(sheet(
    ...
    {{"Target mode", app.target_mode},
     ...
     {"Projection events this cycle", std::to_string(app.projection_events.size())},
  ...
  for (const auto & joint : app.joints) {
    ...
    execution_rows.push_back(
      {joint.name, fixed(joint.execution_position, 5), fixed(joint.execution_velocity, 5),
  ...
  page.sections.push_back(sheet(
    "IK and projected target",
    {textColumn("Joint name"), numberColumn("IK position [rad]"),
     numberColumn("IK velocity [rad/s]"), numberColumn("Target position [rad]"),
     numberColumn("Target velocity [rad/s]"),
     numberColumn("Target acceleration [rad/s\xC2\xB2]"), textColumn("Projection")},
    std::move(target_rows)));
  page.sections.push_back(sheet(
    "Execution and limit utilization",
    {textColumn("Joint name"), numberColumn("Position [rad]"),
     numberColumn("Velocity [rad/s]"), numberColumn("Acceleration [rad/s\xC2\xB2]"),
     numberColumn("Jerk [rad/s\xC2\xB3]"), numberColumn("Velocity utilization [%]"),
     numberColumn("Acceleration utilization [%]"), numberColumn("Jerk utilization [%]")},
    std::move(execution_rows)));
  return page;
}

void replaceJointStatePage(TuiPage & joint_page, const PlannedGroupedStepOtgTuiDebug & app)
{
  ...
  for (const auto & group : groups) {
    const auto maximum = jointGroupMaximum(app.joints, group.second.first, group.second.second);
    maximum_rows.push_back(
      {group.first, fixed(maximum.velocity, 4), fixed(maximum.acceleration, 4),
       fixed(maximum.jerk, 2)});
    utilization_rows.push_back(
      {group.first, fixed(maximum.velocity_utilization, 1),
       fixed(maximum.acceleration_utilization, 1), fixed(maximum.jerk_utilization, 1)});
  }
  joint_page.sections.push_back(sheet(
    "Group maximum motion",
  ...
  joint_page.sections.push_back(sheet(
    "Group maximum limit utilization",
  ...
  for (const auto & joint : app.joints) {
    ...
    limit_rows.push_back(
      {joint.name, fixed(joint.position_lower, 4), fixed(joint.position_upper, 4),
       fixed(utilization(joint.execution_velocity, joint.maximum_velocity), 1),
       fixed(utilization(joint.execution_acceleration, joint.maximum_acceleration), 1),
       fixed(utilization(joint.execution_jerk, joint.maximum_jerk), 1)});
  }
  ...
  joint_page.sections.push_back(sheet(
    ...
    {textColumn("Joint name"), numberColumn("Lower position [rad]"),
     numberColumn("Upper position [rad]"), numberColumn("Velocity utilization [%]"),
     numberColumn("Acceleration utilization [%]"), numberColumn("Jerk utilization [%]")},
...
TuiDocument makeAppTuiDocument(
  ...
  const std::string & input_status)
{
  TuiDocument document = makeStandardIkTuiDocument(
    frame, presentation, publish_count, sink_status, title, input_status);
  document.header_right =
    "source=" + app.source_mode + "  Cartesian planning=" + app.cartesian_plan.state +
    "  Joint execution=" + app.joint_step.state;
  document.footer_hints = app.source_mode == "mcap replay" ?
    "Space pause · . step · 1–7 pages · ? help · x exit" :
    "Space pause · 1–7 pages · ? help · x exit";
  document.help_lines.front() =
    "1..7/F1..F7/Tab/BackTab: pages; PageUp/PageDown/Home/End: scroll; h/?: help";

  auto & overview = page(document, "Overview");
  overview.sections.push_back(sheet(
    "Safety",
    {textColumn("Safety metric"), textColumn("Value")},
    {{"Left task scale", fixed(app.left_task_scale, 3)},
     ...
     {"Maximum absolute acceleration", fixed(app.maximum_absolute_acceleration, 3)},
     {"Maximum absolute jerk", fixed(app.maximum_absolute_jerk, 2)}},
    1U, 0U));
  auto planning = sheet(
    "Planning",
    {textColumn("Planning stage"), textColumn("State"), numberColumn("Calculation [ms]")},
    {{"Cartesian", app.cartesian_plan.state, fixed(app.cartesian_plan.calculation_time_ms, 4)},
     {"Joint plan", app.joint_plan.state, fixed(app.joint_plan.calculation_time_ms, 4)},
     {"Joint first step", app.joint_step.state, fixed(app.joint_step.calculation_time_ms, 4)}},
    1U, 1U);
  ...
  cartesian_page.sections.front() = sheet(
    "Planner",
    {textColumn("State"), numberColumn("Duration [s]"), numberColumn("Sample time [s]"),
     numberColumn("Samples"), numberColumn("Calculation [ms]")},
    {{app.cartesian_plan.state, fixed(app.cartesian_plan.duration_s, 6),
      fixed(app.cartesian_plan.sample_time_s, 6), std::to_string(app.cartesian_plan.sample_count),
  ...
  cartesian_page.sections.insert(
    ...
    sheet(
      ...
      {{"Linear velocity", fixed(app.cartesian_limits.linear_velocity), "m/s"},
       {"Linear acceleration", fixed(app.cartesian_limits.linear_acceleration), "m/s\xC2\xB2"},
       {"Linear jerk", fixed(app.cartesian_limits.linear_jerk), "m/s\xC2\xB3"},
       {"Angular velocity", fixed(app.cartesian_limits.angular_velocity), "rad/s"},
       {"Angular acceleration", fixed(app.cartesian_limits.angular_acceleration), "rad/s\xC2\xB2"},
       {"Angular jerk", fixed(app.cartesian_limits.angular_jerk), "rad/s\xC2\xB3"}}));

  const auto cartesian_position = std::find_if(
    document.pages.begin(), document.pages.end(),
    [](const TuiPage & candidate) {return candidate.title == "Cartesian Planning";});
  ...
  runtime.sections.push_back(sheet(
    ...
    {{"Cartesian", app.cartesian_plan.state, fixed(app.cartesian_plan.duration_s, 6),
     ...
     {"Joint plan", app.joint_plan.state, fixed(app.joint_plan.duration_s, 6),
      fixed(app.joint_plan.sample_time_s, 6), std::to_string(app.joint_plan.sample_count),
      fixed(app.joint_plan.calculation_time_ms, 4)},
     {"Joint first step", app.joint_step.state, fixed(app.joint_step.duration_s, 6),
      fixed(app.joint_step.sample_time_s, 6), std::to_string(app.joint_step.sample_count),
      fixed(app.joint_step.calculation_time_ms, 4)}},
    1U));
  ...
  events.sections.insert(
    ...
    sheet(
      "Joint projection events",
      {textColumn("Joint"), textColumn("Component"), numberColumn("Original"),
  ...
  events.sections.insert(
    ...
    sheet(
      "Cartesian retarget clamp events",
      {textColumn("Arm"), textColumn("Component"), textColumn("Axis"),
       numberColumn("Original"), numberColumn("Applied"), numberColumn("Limit")},
      std::move(clamp_rows)));
  return document;
}
