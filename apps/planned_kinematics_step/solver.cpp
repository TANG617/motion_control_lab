...
Solver::Solver(const Options &opt)
    : o(opt), placo(o.config["solver"].asString() == "placo"),
      names(strings(o.input["joint_names"])),
      active(strings(o.input["active_joint_names"])) {
  if (o.input["model"]["sha256"].asString() !=
      sha256_file(o.input["model"]["locator"].asString()))
...
Result Solver::solve(const std::vector<double> &q, const std::vector<double> &v,
  ...
  if (placo) {
    ...
    out.native["selected_pass"] = Json::nullValue;
    out.native["selected_pass_reason"] = "single weighted solve";
    out.native["native_disposition"] =
        "solve-returned; native enum unavailable";
  ...
  out.native["native_disposition"] = static_cast<int>(s.disposition);
  out.native["selected_pass"] = Json::nullValue;
  out.native["selected_pass_reason"] = "single weighted solve";
