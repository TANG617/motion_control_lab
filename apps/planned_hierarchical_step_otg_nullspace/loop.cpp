...
constexpr const char *kProgramId =
    "mcl_planned_hierarchical_step_otg_nullspace";
constexpr const char *kTitle =
    "Motion Control Planned Hierarchical Step OTG Null-space";
...
struct RedOutputSnapshot {
  ...
  mcl::planned_hierarchical_step_otg_nullspace::JointTarget raw_joint_target;
  mcl::planned_hierarchical_step_otg_nullspace::JointTarget
      projected_joint_target;
  mcl::planned_hierarchical_step_otg_nullspace::ProjectionDiagnostics
      projection;
...
struct RedAttemptSnapshot {
  ...
  mcl::SolverDebug solver_debug;
  mcl::planned_hierarchical_step_otg_nullspace::RetargetClampDiagnostics
      retarget_clamp;
...
const char *retargetClampComponentName(
    mcl::planned_hierarchical_step_otg_nullspace::RetargetClampComponent
        component) {
  using Component =
      mcl::planned_hierarchical_step_otg_nullspace::RetargetClampComponent;
  switch (component) {
...
const char *
projectionFlag(mcl::planned_hierarchical_step_otg_nullspace::ProjectionComponent
                   component) {
  using Component =
      mcl::planned_hierarchical_step_otg_nullspace::ProjectionComponent;
  switch (component) {
...
std::string traceProjectionEvents(
    const mcl::planned_hierarchical_step_otg_nullspace::ProjectionDiagnostics
        &diagnostics,
    const std::vector<std::string> &joint_names) {
  std::ostringstream output;
  for (std::size_t index = 0U; index < diagnostics.events.size(); ++index) {
    ...
    output << joint_names.at(event.joint_index) << ':'
           << mcl::planned_hierarchical_step_otg_nullspace::
...
void fillSelfCollisionDebug(
    ...
    const mcl::planned_hierarchical_step_otg_nullspace::SolverOptions &options,
...
int runLoop(Options planned_options, const R1RobotConfig &robot,
    ...
    if (!replay_options.output_dir_explicit) {
      ...
      const std::filesystem::path output_root =
          replay_options.output_root.value_or(std::filesystem::path{
              "runs/mcl_planned_hierarchical_step_otg_nullspace"});
  ...
  workers.red = std::thread([&]() {
    ...
    mcl::planned_hierarchical_step_otg_nullspace::JointTargetBuilder
        joint_target_builder(planned_options.joint_target,
                             1.0 / options.red_rate_hz, joint_names.size());
    mcl::planned_hierarchical_step_otg_nullspace::ReplaySettlingCounter
    ...
    mcl::runPeriodicWorker(
        ...
        [&](double, std::int64_t sample_time_ns) {
          ...
          const auto mapped_raw_ik =
              mcl::planned_hierarchical_step_otg_nullspace::mapActiveIkToFull(
          ...
          mcl::planned_hierarchical_step_otg_nullspace::ProjectionDiagnostics
              projection;
          auto projected_target = mcl::planned_hierarchical_step_otg_nullspace::
            ...
            if (telemetry_enabled) {
              ...
              red_telemetry->tryPush(telemetryRecord(
                  ...
                  makeCartesianTracking(telemetry_context, target, reference,
                                        staged_for_attempt, raw_left_pose,
  ...
  while (true) {
    ...
    if (schedule->update_due) {
      ...
      mcl::planned_hierarchical_step_otg_nullspace::appendPlanningRequestPoses(
      ...
      mcl::planned_hierarchical_step_otg_nullspace::appendOtgExecution(
      ...
      mcl::planned_hierarchical_step_otg_nullspace::appendNullspaceElbowScene(
  ...
  if (planned_options.replay.has_value()) {
    ...
    joint_otg["algorithm"] = mcl::planned_hierarchical_step_otg_nullspace::
        jointPlanningAlgorithmName(planned_options.planning.joint_algorithm);
    joint_otg["synchronization"] = mcl::
        planned_hierarchical_step_otg_nullspace::planningSynchronizationName(
            planned_options.planning.joint_synchronization);
