...
binary=${MCL_BINARY:-"${install_prefix}/bin/mcl_planned_hierarchical_step_otg_nullspace"}
...
exec "${lab_root}/scripts/launch_app.sh" "${binary}" replay --urdf "${urdf}" \
  ...
  --output-root "${MCL_OUTPUT_ROOT:-${lab_root}/runs/planned_hierarchical_step_otg_nullspace}" --launcher "$0" "$@"
