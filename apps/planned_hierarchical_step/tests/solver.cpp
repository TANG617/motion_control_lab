...
int main() {
  try {
    ...
    require(custom_indices.front() == 1,
            "custom inactive joint mapping mismatch");
    require(std::find(custom_indices.begin(), custom_indices.end(), 4) !=
                custom_indices.end() &&
                std::find(custom_indices.begin(), custom_indices.end(), 5) !=
                    custom_indices.end(),
            "custom inactive joint policy retained hidden waist behavior");
    return EXIT_SUCCESS;
  } catch (const std::exception &error) {
    std::cerr << "planned hierarchical Step solver test failed: "
              << error.what() << '\n';
    return EXIT_FAILURE;
  }
}
