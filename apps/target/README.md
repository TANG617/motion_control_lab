...
`mcl_target` retains its existing keyboard entry and exposes finite JSON batch and
snapshot execution. Both entries construct the existing `MccTargetSolver` or
`PlacoTargetSolver` and call the same `solve` implementation. There is no study solver,
experiment-number branch, hidden acceptance override or fallback.

```bash
BIN=${MCL_INSTALL_PREFIX:-/workspace/install/algorithm}/bin/mcl_target
...
Request schema is `execution_request.v1`, app ID `mcl_target`, execution structure
`target_solve`, JSON input. The request mode rejects extra algorithm CLI flags.
...
- `rate_hz`: default 100; controls application scheduling only, not TargetSolve iterations.
...
```bash
python apps/target/tests/public_execution.py "$BIN" /absolute/canonical.json \
