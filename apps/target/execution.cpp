...
AppOptions options(const Json::Value &c) {
  ...
  keys(a,
       {"regularization", "position_tolerance_m", "orientation_tolerance_rad",
        "joint_position_margin_rad", "maximum_iterations",
        "soft_solve_time_budget_ms", "minimum_position_improvement_m",
        "minimum_orientation_improvement_rad", "posture_weight",
        "proxqp_absolute_tolerance"});
  ...
  if (a.isMember("maximum_iterations"))
    o.algorithm.maximum_iterations = a["maximum_iterations"].asInt();
  if (a.isMember("soft_solve_time_budget_ms"))
    o.algorithm.soft_solve_time_budget_ms =
        a["soft_solve_time_budget_ms"].asDouble();
  if (a.isMember("minimum_position_improvement_m"))
    o.algorithm.minimum_position_improvement_m =
        a["minimum_position_improvement_m"].asDouble();
  if (a.isMember("minimum_orientation_improvement_rad"))
    o.algorithm.minimum_orientation_improvement_rad =
        a["minimum_orientation_improvement_rad"].asDouble();
  if (a.isMember("posture_weight"))
    o.algorithm.posture_weight = a["posture_weight"].asDouble();
  if (a.isMember("proxqp_absolute_tolerance"))
    o.algorithm.proxqp_absolute_tolerance =
        a["proxqp_absolute_tolerance"].asDouble();
...
Json::Value resolve(const AppOptions &o) {
  ...
  r["algorithm"]["joint_position_margin_rad"] =
      o.algorithm.joint_position_margin_rad;
  r["algorithm"]["maximum_iterations"] = o.algorithm.maximum_iterations;
  r["algorithm"]["soft_solve_time_budget_ms"] =
      o.algorithm.soft_solve_time_budget_ms;
  r["algorithm"]["minimum_position_improvement_m"] =
      o.algorithm.minimum_position_improvement_m;
  r["algorithm"]["minimum_orientation_improvement_rad"] =
      o.algorithm.minimum_orientation_improvement_rad;
  r["algorithm"]["posture_weight"] = o.algorithm.posture_weight;
  r["algorithm"]["proxqp_absolute_tolerance"] =
      o.algorithm.proxqp_absolute_tolerance;
  return r;
}
...
int batch(ex::Request &r, const AppOptions &o, Solver &solver, double init_ms) {
  ...
  mapping["velocity_limits_enforced"] = false;
...
Json::Value capabilities() {
  ...
  c["app_id"] = "mcl_target";
  c["app_version"] = "2";
  c["execution_structures"].append("target_solve");
...
int executePublic(int argc, char **argv) {
  ...
  if (ex::isRequest(argc, argv)) {
    ...
    r = ex::readRequest(argv[2], "mcl_target");
  } else {
    ...
    r.document["app_id"] = "mcl_target";
    r.document["execution_structure"] = "target_solve";
  ...
  if (r.document["execution_structure"] != "target_solve" ||
      r.document["input"]["format"] != "json")
  ...
  if (o.solver == SolverKind::Mcc) {
    MccTargetSolver solver(o.interactive.urdf_path, robot, o.backend,
                           o.algorithm);
  ...
  PlacoTargetSolver solver(o.interactive.urdf_path, robot, o.algorithm);
  return batch(r, o, solver,
               std::chrono::duration<double, std::milli>(
                   std::chrono::steady_clock::now() - started)
                   .count());
