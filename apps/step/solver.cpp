...
class MccServoSolver::Impl {
public:
  Impl(const std::string &urdf_path, double rate_hz,
       const mcl::R1RobotConfig &robot, MccBackend backend,
      ...
      : robot_(robot), backend_(backend),
        positions_(initial_positions.empty() ? robot.default_positions
                                             : initial_positions),
        velocities_(initial_velocities.empty()
                        ? std::vector<double>(positions_.size(), 0.0)
                        : initial_velocities) {
    ...
    if (observer_) {
      ...
      if (full_observation_) {
        ...
        for (const auto &task : linear.tasks) {
          ...
          row["decision_unit"] = "joint velocity rad/s";
          if (const auto *hard =
...
public:
  Impl(const std::string &urdf_path, double rate_hz,
       ...
       const std::vector<double> &initial_positions = {},
       const std::vector<double> &initial_velocities = {})
      : robot_config_(robot), algorithm_(algorithm),
        positions_(initial_positions.empty() ? robot.default_positions
                                             : initial_positions),
        velocities_(initial_velocities.empty()
                        ? std::vector<double>(positions_.size(), 0.0)
  ...
  ServoSolveResult solve(const std::vector<mcl::ArmTarget> &targets) {
    ...
    if (observer_) {
      ...
      raw["native_dimensions"]["inequalities"] = solver_.problem.n_inequalities;
      ...
      raw["returned_solution_role"] = "native-wrapper-state-before-app-commit";
      raw["candidate_velocity_role"] = "native-servo-wrapper-velocity";
      observer_(raw);
    }
    robot_.update_kinematics();
    ...
    for (std::size_t index = 0; index < robot_config_.joint_names.size();
      ...
      velocities_[index] =
          robot_.get_joint_velocity(robot_config_.joint_names[index]);
    }
    ...
    const bool converged =
        left_error.position_m <= algorithm_.position_tolerance_m &&
    ...
    result.solver_debug.joint_limit_policy =
        "model-position+velocity (margin 1e-3)";
    result.solver_debug.termination_reason =
        converged ? "converged" : "single-iteration";
...
private:
  mcl::step::AlgorithmOptions algorithm_;
...
MccServoSolver::MccServoSolver(const std::string &urdf_path, double rate_hz,
                               const R1RobotConfig &robot, MccBackend backend,
                               const AlgorithmOptions &algorithm,
                               const std::vector<double> &initial_positions,
                               const std::vector<double> &initial_velocities)
    : impl_(std::make_unique<Impl>(urdf_path, rate_hz, robot, backend,
                                   algorithm, initial_positions,
                                   initial_velocities)) {}

MccServoSolver::~MccServoSolver() = default;

const std::vector<double> &MccServoSolver::positions() const {
  return impl_->positions();
}

const std::vector<double> &MccServoSolver::velocities() const {
  return impl_->velocities();
}

Pose MccServoSolver::currentPose(ArmSide side) {
  return impl_->currentPose(side);
}

ServoSolveResult MccServoSolver::solve(const std::vector<ArmTarget> &targets) {
  return impl_->solve(targets);
}

PlacoServoSolver::PlacoServoSolver(
    const std::string &urdf_path, double rate_hz, const R1RobotConfig &robot,
    const AlgorithmOptions &algorithm,
    const std::vector<double> &initial_positions,
    const std::vector<double> &initial_velocities)
    : impl_(std::make_unique<Impl>(urdf_path, rate_hz, robot, algorithm,
                                   initial_positions, initial_velocities)) {}

PlacoServoSolver::~PlacoServoSolver() = default;

const std::vector<double> &PlacoServoSolver::positions() const {
  return impl_->positions();
}

const std::vector<double> &PlacoServoSolver::velocities() const {
  return impl_->velocities();
}

Pose PlacoServoSolver::currentPose(ArmSide side) {
  return impl_->currentPose(side);
}

ServoSolveResult
PlacoServoSolver::solve(const std::vector<ArmTarget> &targets) {
  return impl_->solve(targets);
}

void MccServoSolver::setState(const std::vector<double> &q,
                              const std::vector<double> &v) {
  impl_->setState(q, v);
}
void MccServoSolver::observe(std::function<void(const Json::Value &)> observer,
...
void PlacoServoSolver::setState(const std::vector<double> &q,
                                const std::vector<double> &v) {
  impl_->setState(q, v);
}
void PlacoServoSolver::observe(
...
Json::Value MccServoSolver::modelMapping() const {
  return impl_->modelMapping();
}
Json::Value PlacoServoSolver::modelMapping() const {
  return impl_->modelMapping();
}
} // namespace motion_control_lab::step
