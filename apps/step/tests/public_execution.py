...
def run(binary,source,root):
    ...
    cap=json.loads(call(['--describe-capabilities']).stdout);assert cap['app_id']=='mcl_step'
    for solver,backend in [('mcc','eiquadprog'),('mcc','proxqp'),('placo','eiquadprog')]:
      for mode in ['minimal','full','cpp-new']:
        ...
        req={'schema_version':'execution_request.v1','app_id':'mcl_step','execution_structure':'servo_step','input':{'path':str(ip),'sha256':hashlib.sha256(ip.read_bytes()).hexdigest(),'format':'json'},'app_config':config,'execution':execution,'observation':{'mode':mode},'tracking':{'opaque':'not-an-algorithm-selector'},'output_dir':str(reqout)}
    ...
    request={'schema_version':'execution_request.v1','app_id':'mcl_step','execution_structure':'servo_step','input':{'path':str(ip),'sha256':hashlib.sha256(ip.read_bytes()).hexdigest(),'format':'json'},'app_config':json.loads(cp.read_text()),'execution':json.loads(ep.read_text()),'observation':{'mode':'minimal'},'tracking':{},'output_dir':str(root/'negative-minimal')}
    ...
    print('mcl_step: 9 normal/request/observation pairs, windows, integrity rejection and native failure PASS')
