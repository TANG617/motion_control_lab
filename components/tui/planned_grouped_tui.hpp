...
struct PlannedGroupedTuiConfig {
  bool enabled{true};
};
...
struct PlannedGroupedJointOtgTuiDebug {
...
struct PlannedGroupedTuiSnapshot {
  const IkDebugFrame *frame{nullptr};
  const InteractiveIkPresentation *presentation{nullptr};
  std::optional<PlannedGroupedJointOtgTuiDebug> joint_otg;
...
class PlannedGroupedTui {
public:
  explicit PlannedGroupedTui(PlannedGroupedTuiConfig config);

  void handleNavigation(const KeyEvent &event);
  void render(const PlannedGroupedTuiSnapshot &snapshot);
