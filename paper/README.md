...
A01/A02 exploratory analyses of fixed historical non-RT evidence are implemented. Their results do not transfer to the new app methods. A03, formal statistical analysis, publication builds and manuscript drafting remain deferred; current status is maintained in the [study index](../docs/mcc_placo_study/README.md). Protocols, metric definitions, claim IDs and figure specifications remain preregistered now. Campaign closure records every required unit, including failures and unavailable inputs; it does not require positive results or imply evidence completeness.
