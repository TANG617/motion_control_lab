...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="e01-", dir=args.work_root) as temporary:
        ...
        for row in trace:
            for field, value in row.items():
                if not math.isfinite(float(value)):
                    raise ValueError(f"non-finite E01 trace value in {field}")
        ...
        if int(trace[-1]["iteration"]) > maximum_iterations:
            raise ValueError("PlaCo exceeded the declared iteration limit")
        ...
        for metric in metrics:
            if not math.isfinite(float(metric["value"])):
                raise ValueError(f"non-finite E01 metric: {metric['metric_id']}")
        guardrail = next(
            row for row in metrics if row["metric_id"] == "joint_limit_violation_count"
        )
        if float(guardrail["value"]) != 0.0:
            raise ValueError("PlaCo joint-limit guardrail failed")
