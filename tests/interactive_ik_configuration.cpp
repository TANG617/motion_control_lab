...
int main()
{
  ...
  if (!visualization.joints.has_value() ||
      visualization.run_id != "interactive-preview-placo" ||
      ...
      diagnosticValue("ik.solve_time.p90") != 0.1 ||
      diagnosticValue("ik.solve_time.p95") != 0.2 ||
      diagnosticValue("ik.solve_time.p99") != 0.3 ||
      diagnosticValue("ik.solve_attempts") != 4.0 ||
      diagnosticValue("ik.accepted") != 3.0 ||
      diagnosticValue("ik.rejected") != 1.0) {
    return EXIT_FAILURE;
  }
