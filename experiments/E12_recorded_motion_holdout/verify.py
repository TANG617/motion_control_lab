...
def verify(request):
 ...
 source_end=inp['samples'][-1]['source_time_s'];source_ticks=[r for r in trace if r['source_time_s']<=source_end];settling_ticks=[r for r in trace if r['source_time_s']>source_end]
 observed_sources=len({r['input_sequence'] for r in source_ticks});required_sources=len(inp['samples']);source_coverage=observed_sources/required_sources if required_sources else None
 summary=dict(recording_id=inp['input_id'],session_id=inp.get('session_id','unknown'),split_id=inp.get('split_id','unknown'),method_id=request['identity']['method_id'],attempts=count,holds=holds,source_coverage=source_coverage,settling_observed_s=len(settling_ticks)*cfg['dt_s'],settling_status='thresholds-unavailable' if settling_ticks else 'not-reached',status='partial' if count!=expected or holds else 'completed-native-window',valid_completion='unavailable: formal quality and settling thresholds not frozen')
 with (out/'recording_summary.csv').open('w') as f:w=csv.DictWriter(f,list(summary));w.writeheader();w.writerow(summary)
 with (out/'paired_coverage.csv').open('w') as f:w=csv.DictWriter(f,['case_id','method_id','input_hash','pair_status','reason']);w.writeheader();w.writerow(dict(case_id=request['identity']['case_id'],method_id=request['identity']['method_id'],input_hash=request['identity']['input_hash'],pair_status='unavailable',reason='unit-local evidence; pair only matching frozen identities after campaign'))
 write_json(out/'failures.json',dict(native_rejected_attempts=[r['attempt_sequence'] for r in trace if r.get('ik_disposition')=='rejected'],execution_hold_attempts=[r['attempt_sequence'] for r in trace if r['execution_state']=='HOLD'],validation_issues=issues,settling=summary['settling_status']))
