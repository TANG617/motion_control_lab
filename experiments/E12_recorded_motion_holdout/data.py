...
if __name__=='__main__':
 p=argparse.ArgumentParser();sub=p.add_subparsers(dest='command',required=True);i=sub.add_parser('inventory');i.add_argument('--locator',default='/workspace/fixtures/raw/batch');i.add_argument('--metadata');i.add_argument('--output',required=True);c=sub.add_parser('convert');c.add_argument('--inventory',required=True);c.add_argument('--split',required=True);c.add_argument('--recording',required=True);c.add_argument('--recipe',required=True);c.add_argument('--model',required=True);c.add_argument('--output',required=True);c.add_argument('--phase',choices=['development','confirmatory'],default='development');c.add_argument('--frozen-definition');a=p.parse_args()
