#!/usr/bin/env python3
"""Forward E08 verifier v2: exact final selected scale; v1 artifacts remain immutable.

The independent module intentionally preserves the frozen v1 implementation byte-for-byte.
Normal future declarations call --request. To inspect existing evidence without touching its
run, supply --request OLD/request.json --raw OLD/raw.jsonl --output NEW_DIRECTORY.
This invokes no candidate app; a real committed violation still returns exit code 1.
"""
...
VERIFIER_VERSION="e08-verifier.v2"
...
def selected_scale(row,task,method):
    """Exact selected-candidate operand, not a primary optimum or absolute drift."""
    if task.get('enforcement')!='scaled':return dict(value=1.,primary_optimum=None,status='unscaled',source='unscaled equation')
    scales=[s for s in row.get('scales',[]) if s.get('handle')==task.get('scale_group_handle') and s.get('evaluated')]
    if len(scales)!=1:return dict(value=None,primary_optimum=None,status='unavailable',reason='missing or duplicate scale identity')
    optimum=scales[0]
    base=dict(primary_optimum=optimum['value'],value=None,status='unavailable')
    if not row.get('evidence_linearization',{}).get('candidate_available'):
        return dict(base,reason='no captured native selected candidate')
    if 'hqp' in method:
        selected=row.get('selected_priority')
        passes=[p for p in row.get('passes',[]) if selected is not None and p.get('pass')==selected and p.get('attempted') and p.get('succeeded')]
        final=[c for c in row.get('native_constraints',[]) if c.get('name')==optimum.get('name') and c.get('component_name')=='scale']
        if len(passes)!=1:return dict(base,reason='missing or duplicate successful selected pass')
        if len(final)!=1:return dict(base,reason='exact final scale missing; absolute drift does not determine its sign')
        value=final[0]['value'];source='native_constraints exact selected pass scale'
        base['selected_pass']=passes[0]
    else:
        if row.get('completed_passes')!=1:return dict(base,reason='weighted successful final pass unavailable')
        value=optimum['value'];source='weighted final OptimizationDiagnostics scale'
    if not isinstance(value,(float,int)) or not np.isfinite(value):return dict(base,reason='nonfinite final scale')
    return dict(base,value=value,status='measured',source=source)


def audit(request,raw_path=None,output_path=None):
    request_path=Path(request);req=C.read_json(request_path);out=Path(req['output_dir']);data=req['input'];config=req['config']
    if (raw_path is None)!=(output_path is None):raise ValueError('offline recheck requires both explicit raw and a new output directory')
    if output_path is not None:
        out=Path(output_path).resolve();out.mkdir(parents=True,exist_ok=False)
    ...
    raw=Path(raw_path).resolve() if raw_path is not None else out/'raw.jsonl';sources=[C.artifact(request_path),C.artifact(raw)] if raw.exists() else [C.artifact(request_path)]
    sources.extend(C.artifact(p) for p in (Path(__file__).resolve(),HERE.parents[1]/'tools/mcc_placo_study/repair/kinematics.py',HERE.parents[1]/'tools/mcc_placo_study/inputs.py',Path(C.__file__).resolve()))
        ...
        if raw.exists():
            for line,row in enumerate(C.jsonl_rows(raw),1):
                ...
                for task in task_rows:
                    ...
                    scale_evidence=selected_scale(row,task,config['evidence_original_method'])
                    scale=scale_evidence['value']
                    ...
                    legacy=residual_record(geo,desired,baseline,row.get('candidate_active_v'),scale_evidence.get('primary_optimum') if task['enforcement']=='scaled' else 1.,tol)
                    ...
                    info=dict(source_line=line,attempt_sequence=seq,source_time_s=row.get('source_time_s'),task=task['name'],side=side,
                        ...
                        max_A_difference=maximum(A-geo),max_b_difference=maximum(b-desired),baseline_velocity=baseline.tolist(),scale=scale,scale_evidence=scale_evidence,verifier_version=VERIFIER_VERSION,
    ...
    summary=dict(schema_version='e08_evidence_audit.v2',verifier_version=VERIFIER_VERSION,
        operand_policy='same attempt captured native candidate and exact final selected scale; primary optimum only for legacy returned-state diagnostic',observation_complete=complete,expected_attempts=expected,
        ...
        finite_difference_eps=list(FD_EPS),sources=sources,numpy_version=np.__version__,offline_read_only_raw=raw_path is not None,
...
if __name__=='__main__':
    ...
    parser.add_argument('--raw',help='read-only existing raw for an offline recheck');parser.add_argument('--output',help='new directory for offline recheck; existing directories are rejected')
    args=parser.parse_args();raise SystemExit(audit(args.request,args.raw,args.output))
