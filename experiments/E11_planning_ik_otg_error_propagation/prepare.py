...
import argparse,copy,math,pathlib,sys,json
...
def smooth_wave(t):
 # Continuous position, velocity and acceleration at every 2.5s knot.
 knots=(0.,1.,0.,-1.,0.);segment=int(t/2.5)%4;u=(t%2.5)/2.5
 blend=10*u**3-15*u**4+6*u**5
 return knots[segment]+(knots[segment+1]-knots[segment])*blend
def trajectory(model,amplitude,frequency,family,fixture=False):
 ...
 for seq in range(1001):
  ...
  if family=='step':wave=0. if t<2 else 1.;event='step' if 2<=t<2.01 else 'motion'
   ...
   wave=smooth_wave(t);event='transition' if t%2.5<.01 else 'motion'
...
def prepare(model,out):
 ...
 source=pathlib.Path(__file__).read_bytes();source_hash=__import__('hashlib').sha256(source).hexdigest();snapshot=out/('generator-'+source_hash[:16]+'.py');snapshot.write_bytes(source)
 ...
 for policy,label in [('explicit-position-bounds','projection'),('none','otg-rejection')]:
  ...
  units.append(dict(case_id='fixture-'+label,method_id='mcc-hqp-2',arm_id='mcc-hqp-2-joint',input=fixture_descriptor,config=cfg,required=True,binary='mcl_study_e11',smoke_config={'duration_s':.003},validation_command=[sys.executable,str(pathlib.Path(__file__).with_name('verify.py')),'--request','{request}']))
 ...
 validate_definition(d);write_json(pathlib.Path(__file__).with_name('definition.json'),d);return d
