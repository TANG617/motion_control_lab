...
RELEASE_WORKERS=('primary','secondary','target','feedback-capture','output')

def progress_total(events,attempts):
    """Record checks: release accounting, primary attempts, then output joins.

    Output records count again for their distinct command/hold verification.
    """
    return (sum(e.get('worker') in RELEASE_WORKERS for e in events)
            +sum(r.get('worker')=='primary' for r in attempts)
            +sum(e.get('worker')=='output' and not e.get('skipped') for e in events))
...
def check(events,attempts,config,native,initial_state=None,progress=None):
    failures=[];duration=round(config['duration_s']*1e9);counts={};misses={}
    completed=0
    def checked():
        nonlocal completed
        completed+=1
        if progress is not None:progress.update(completed)
    ...
    previous_revision=0
    publications_by_revision=None;captures_by_state=None
    for r in attempts:
        if r.get('worker')!='primary':continue
        if 'attempt_sequence' not in r:
            failures.append('primary-missing-attempt_sequence');checked();continue
        ...
        if publications_by_revision is None:
            publications_by_revision={}
            for event in events:
                if event.get('worker')=='proposal-publication':
                    publications_by_revision.setdefault(event['revision'],[]).append(event)
        publications=[e for e in publications_by_revision.get(r['proposal_revision'],[]) if e.get('publication_ns',e['delivery_ns'])<=r['start_ns']]
        ...
        if 'measured_q' in r:
            if captures_by_state is None:
                captures_by_state={}
                for event in events:
                    if event.get('worker')=='feedback-capture':
                        captures_by_state.setdefault((event.get('sequence'),event.get('capture_ns')),[]).append(event)
            captures=captures_by_state.get((r['state_sequence'],r['measured_state_capture_ns']),[])
            if captures:
                capture=captures[-1]
...
if __name__=='__main__':
    ...
    result=check(events,attempts,req['config'],json.load(open(out/'native_status.json')),req['input']['initial_state'],SampleProgress(out,'app-validation',progress_total(events,attempts)));write_json(out/'schedule_validation.json',result);sys.exit(not result['passed'])
